...
1. 验证访问：
    ...
    构建并运行docker后：
   - Web界面：浏览器访问`http://localhost:8501`，能看到登录/上传页面即成功。（网页运行可能需要等待一段时间）
...
3. 如果不用docker，也可以直接在本地运行，运行方式为：在终端中进入根目录，先在一个终端中运行`python main.py --mode api`，再在另一个终端运行 `streamlit run web_app.py --server.port=8501`，再打开网页`http://localhost:8501`。
